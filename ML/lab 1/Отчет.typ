...
#show heading: it => {
  it
  par(first-line-indent: 1cm, leading: 0.1em)[#text(size: 0pt)[ ]]
}
...
Цель работы: Разработать модели машинного обучения для прогнозирования арендной платы за жилье на основе предоставленных данных.
...
Данные загружены из файла `House_Rent_Dataset.csv`.
...
Предобработчик: `ColumnTransformer` с `OneHotEncoder` для категориальных и `remainder='passthrough'` для числовых.

= 3. Разделение набора данных

Набор данных разделен на обучающую (80%) и тестовую (20%) выборки с помощью `train_test_split` (random_state не указан явно, но подразумевается воспроизводимость).

X: Все признаки кроме `Rent`, `Posted On`, `Area Locality`, `Floor`.
y: `Rent`.
...
- Ридж-регрессия: `Ridge` (alpha=1.0 по умолчанию).

Модели объединены в пайплайны с предобработчиком.

= 5. Прогнозные значения

Прогнозы вычислены с помощью `predict` на тестовой выборке для каждой модели.

= 6. График регрессии, прогнозные значения и уравнение регрессии
